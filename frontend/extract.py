...
class Extractor():
    def _init_(self, url, html, threshold, output, **kwargs):
        self.url = url
        self.title = ''
        self.html = html
        self.output = output
    ...
    def parse(self) -> tuple:
        ...
        if soup:
            ...
            if self.output == 'markdown':
                return self._get_title(article_html), self._html_to_md(article_html)
            else:
                return self._get_title(article_html), self._clean_html(article_html)
        return '', ''
